...
for condition = 1:length(conditions)
    ...
    for trial = 1:length(trials)
        ...
        for leg = 1:length(legs)
            ...
            SCI_subjects.(conditions{condition}).(trials{trial}).Event.(legs{leg}).HS_marker = ceil((time_points_for_HS / coefficient_dilation_marker) * fs_marker);
            SCI_subjects.(conditions{condition}).(trials{trial}).Event.(legs{leg}).TO_marker = ceil((time_points_for_TO / coefficient_dilation_marker) * fs_marker);
            
            
            SCI_subjects.(conditions{condition}).(trials{trial}).Event.(legs{leg}).HS_emg = ceil((time_points_for_HS / coefficient_dilation_emg) * fs_emg);
            SCI_subjects.(conditions{condition}).(trials{trial}).Event.(legs{leg}).TO_emg = ceil((time_points_for_TO / coefficient_dilation_emg) * fs_emg);
        end
    end
end
